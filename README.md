...
- **아트 패스 5 (실사 패스, 7회 반복)** — 근접 시점을 추가해 판정. 건물 방향·기초(회전 코드가
  아예 없었다), 풍화 텍스처(물이 지나간 자리가 더럽다), 전장 팔레트 채도, 덤불 실루엣,
  **마을 군집**, 도로 갓길·배수로. 실사는 폴리곤이 아니라 재질·접지·배치에서 온다
- **세로 플레이** — 가로 고정을 풀었다. 세로에서는 영상을 늘리지 않고 **나눈다**:
  상단 16:9 영상 + 하단 조종 데크(계기·무전·스틱). 실제 지상국 배치이고,
  계기가 데크로 내려가 **영상이 가로보다 깨끗하다**
- **웹 제스처 차단** — 두 엄지로 스틱을 잡으면 브라우저가 핀치로 읽어 화면이 줌/스크롤되던 것.
  `touch-action: none` + `overscroll-behavior` + iOS `gesturestart` 차단(3겹).
  멀티터치 자체는 원래 정상이었다 — 브라우저가 가로챈 것이었다
- Playwright **115종** 통과, 드로우콜 90 / 삼각형 279k
