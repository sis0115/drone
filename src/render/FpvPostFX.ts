...
void main(){
  ...
  if(uThermal>1.5){
    ...
    // 주간 컬러 — 아트 패스 3: 채도 45%→62% + lift 축소 (실사화).
    // 저채도·들린 블랙이 "뿌연 화면"의 주범이었다. 대비 0.92 와 하이라이트 뭉갬 0.18 은
    // 유지한다 — 1.0/0.12 로 풀었더니 밝은 지면이 백색으로 클리핑됐다(1차 실측).
    vec3 sat = mix(vec3(lum), c, 0.62);
    sat = (sat-0.5)*uP[13]*0.92+0.5;
    sat = mix(sat, vec3(1.0)-((vec3(1.0)-sat)*(vec3(1.0)-sat)), 0.18);
    col = sat * vec3(0.995,1.0,0.97) + vec3(0.014,0.016,0.014);
  } else {
    // 아날로그 흑백 (녹색 인광)
