...
> **왜곡 계수는 출처가 하나여야 한다.** `postfx.distort` 는 `CAMERA.DISTORT` 를 참조한다.
> 상수를 두 벌 들고 있으면 튜닝 패널로 한쪽만 바뀌어 **락온 박스가 조용히 어긋난다**
> (실제로 그렇게 되어 있던 것을 T5 에서 잡았다). `tests/overlay.spec.ts` 가 강제한다.
>
> 역변환은 **뉴턴법 2회로 정확히 푼다.** 프로토타입의 1차 근사 `t·(1-k|t|²)` 는
> 화면 가장자리에서 왕복 오차가 **2.35%** 까지 벌어져 마커가 표적을 벗어났다(실측).
```
...
| **T4** ✅ | 입력(고정 패드/키보드/스크립트) + InputSource + 어시스트 3단계 | 8자 비행 테스트 통과, 패드→비행 브라우저 검증 |
| **T5** ✅ | HUD + 표적 오버레이 (배럴 왜곡 역변환 포함) + 표적(트럭) | 락온 박스 정렬 확인, 왜곡 왕복 오차 <0.05% |
