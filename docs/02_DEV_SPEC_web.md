...
| **T8a** ✅ | **자폭 돌입 판정** — 표적 격파 + 확인(BDA) + SIGNAL LOST 연출 | 트럭에 부딪혀 격파되고, 격파 사실이 화면과 이벤트에 남는다 |
| **T8b** ✅ | 작전 구역(AO) 경계 + 이탈 경고 | 경계 밖 3초 → 임무 실패. 맵 끝이 화면에 안 보인다 |
| **T8c** ✅ | 미션 러너 + `MissionDef` + 목표/실패 판정 + 디브리핑(실패 원인 1줄) | M2 1차수 클리어 가능 |
| **T9** ✅ | SP 경제 + 재고제 + 저장 + 로드아웃 화면 | 재접속 시 SP·재고 유지 |
| **T10** ◐ | i18n(CSV 로드) + 성능 패스 | 언어 전환 ✅ · 중급 폰 60fps 는 **실기 측정 대기**(컨테이너 검증 불가) |
...
> **T8 을 셋으로 쪼갠 이유** (2026-08-26 점검 스윕):
> 15개 상태를 찍어 보고 나서야 드러난 것이 있다 — **표적을 격파할 방법이 없다.**
> 락온 박스는 뜨는데 `input.fire` 는 아무 데도 연결돼 있지 않고, `HIT_RADIUS`(4.2/7.0m)는
> 정의만 되어 있고 쓰이지 않는다. 즉 지금 게임은 "날아다니다 죽는 것"까지만 된다.
> 핵심 루프(표적 격파 → 확인 → SP)의 **첫 칸이 비어 있는데** 그 위에 미션 러너와 경제를
> 얹으면, 러너가 판정할 대상이 없어 껍데기가 된다.
> 그래서 격파 판정(T8a)을 러너보다 **앞에** 뺐다. T8b(작전 구역)도 마찬가지다 —
> GDD M2 의 실패 조건이 "표적 이탈(맵 밖)"인데 지금은 경계 자체가 없고,
> 멀리 나가면 지형이 끊긴 자리가 화면에 그대로 보인다(`sweep-10-weak-signal.png`).
> 러너가 "실패"를 판정하려면 실패할 수 있는 경계가 먼저 있어야 한다.

> **T8a 설계 메모 — 무장이 아니라 기체가 탄이다.**
> GDD 4장 시그니처가 "격추·자폭 순간 컬러바 → 노이즈 → SIGNAL LOST" 이고
> 4.7 이 "1기 자폭 = 미션 종료" 를 전제한다. 즉 FPV 자폭 드론이므로
> **조준·발사가 아니라 돌입이 공격**이다. `HIT_RADIUS` 가 어시스트별로 갈라져 있는 것도
> (아케이드 7.0 / 프로 4.2) 그 뜻이다 — 어시스트를 끄면 더 정확히 박아야 한다.
> `input.fire` 는 M6 투하 미션의 페이로드 투하용으로 남겨 둔다.
